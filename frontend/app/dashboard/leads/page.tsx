      ...
      <div className="mb-8 flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            {leads.length} leads • sorted by {sortBy}
          </p>
        </div>
      ...
      {leads.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[...leads]
            ...
            <div
              key={lead.id}
              className={`bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 hover:shadow-lg transition-shadow ${score >= 80 ? 'ring-2 ring-green-500' : ''}`}
            >
              {/* Score Badge */}
              <div className="flex items-center justify-between mb-3">
                <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-sm font-bold ${getScoreColor(score)}`}>
                  <FireIcon className="w-4 h-4" />
                  {score} - {getScoreLabel(score)}
                </div>
                {score >= 80 && <span className="text-xs text-green-600 animate-pulse">🔥 Call Now!</span>}
              </div>
              {/* Lead Name */}
